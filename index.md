...
[그래픽]
1. 3가지 맵을 고르는 화면이 있다.
2. 맵을 클릭하고 시작버튼을 누르면 게임이 시작된다.
3. 플레이어가 뛰는 길거리 양 옆으로 WesternMap, Street, Hood의 분위기로 길거리와 바닥, 하늘, 장애물도 다르게 제작
4. 맵에 따라 Ai의 모습, 총기를 달리 한다.
5. 플레이어 화면 가운데 위에는 제한시간을 표시한다.

[프로그래밍]
1. 게임이 시작되면 해당 맵의 가운데 라인에 플레이어가 서있고(1인칭) 5초의 카운트 후 게임이 시작된다.
2. 플레이어의 화면 밑에 부분에는 길게 플레이어의 HP가 보인다.
3. 플레이어는 WASD로 이동하며 마우스로 시점을 회전 할 수 있다.
4. Shift를 누르면 플레이어가 보는 방향으로 플레이어가 구른다(쿨타임 6초).
5. Ctrl을 누르면 제자리에 앉는다(Ctrl을 누르고 있는 동안은 앉는 자세가 유지된다 쿨타임 2초).
6. player 1,2,3에 따라 이동속도와 HP값을 달리 한다.
7. Ai 1,2,3에 따라 총의 발사속도, 발사하는 탄의 개수를 달리 한다.
8. 플레이어의 위치에 레이저 및 총을 발사한다.
9. 플레이어가 움직일 수 있는 바닥과 그 위 장애물은 Ai쪽에서 플레이어쪽으로 이동한다.
10. 플레이어와 Ai의 거리는 항상 같게 제한하되 맵의 끝자락에 오면 제한이 풀리게 한다.
11. 플레이어가 피격 시 무적타임을 주고 플레이어의 HP를 줄인다.
12. 플레이어가 파란색 레이저에 피격 시 플레이어의 최대 HP를 넘기지 않게 한다.
13. 맵에 스폰되는 장애물에 레이저와 총알은 막힌다.
14. 제한시간 혹은 플레이어의 HP가 0이 되었을 경우 Game Over UI를 띄우고 게임을 멈춘다.
15. Game Over UI 밑에는 다시하기와 나가기 버튼이 있다.
...
5. 맵에 따라 Ai의 모습, 총기를 달리 한다.
6. 
7. 플레이어 화면 가운데 위에는 제한시간을 표시한다.
...
4. Shift를 누르면 플레이어가 보는 방향으로 플레이어가 구른다(쿨타임 6초).
5. Ctrl을 누르면 제자리에 앉는다(Ctrl을 누르고 있는 동안은 앉는 자세가 유지된다 쿨타임 2초).
6. player 1,2,3에 따라 이동속도 HP값을 달리 한다.
7. Ai 1,2,3에 따라 총의 발사속도, 발사하는 탄의 개수를 달리 한다.
8. 플레이어의 위치, 랜덤한 위치 혹은 두곳 모두에 랜덤으로 레이저 및 총을 발사한다.
9. 빨간 레이저는 체력 감소, 파란 레이저는 체력 회복하게 한다.
10. 플레이어가 움직일 수 있는 바닥과 그 위 장애물은 Ai쪽에서 플레이어쪽으로 이동한다.
11. 플레이어와 Ai의 거리는 항상 같게 제한하되 맵의 끝자락에 오면 제한이 풀리게 한다.
12. 플레이어가 피격 시 잠깐의 정지와 무적타임을 주고 플레이어의 HP를 줄인다.
13. 플레이어가 파란색 레이저에 피격 시 플레이어의 최대 HP를 넘기지 않게 한다.
14. 맵에 스폰되는 장애물에 레이저와 총알은 막힌다.
15. 제한시간 혹은 플레이어의 HP가 0이 되었을 경우 Game Over UI를 띄우고 게임을 멈춘다.
16. Game Over UI 밑에는 다시하기와 나가기 버튼이 있다.
17. 다시하기를 누를 경우 맵의 처음으로 돌아와 게임을 시작한다.
18. 나가기를 누를 경우 맵 선택 화면으로 돌아간다.
19. 목표지점에 도달할 경우 승리 Scene으로 넘어간다.
20. 3D 1인칭 뷰 게임에서 VR로 제작을 한다.
21. 제한시간의 1/2, 제한시간의 3/4마다 장애물 생성 수를 줄인다.
